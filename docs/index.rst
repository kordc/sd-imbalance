.. ImbalanceSD documentation master file, created by
   sphinx-quickstart on Fri May 23 00:57:44 2025.
...
3.  **Run a Training Experiment:**
    ...
    (Don't forget to configure your WandB account for experiment tracking! See `WandB Account Configuration`_ below.)
...
*   **Windows:**
    ...
    .. note::
        Note, that currently you may hit a bug with `uv` on Windows. If you do, please use WSL instead. You can install WSL by following the instructions `here <https://docs.microsoft.com/en-us/windows/wsl/install>`_.
...
1.  **Docker Engine & Docker Compose**:
    *   **Linux (Ubuntu)**: Follow the official Docker Engine installation guide: `Install Docker Engine on Ubuntu <https://docs.docker.com/engine/install/ubuntu/>`_.
2.  **NVIDIA Container Toolkit (for GPU acceleration)**:
    *   If you have an NVIDIA GPU and want to use it for training (highly recommended), you **must** install the `NVIDIA Container Toolkit <https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html>`_ on your host system. This allows Docker to access your GPU.
...
1.  Go to `https://wandb.ai/authorize <https://wandb.ai/authorize>`_ to get your API key.
2.  Set it as a system environment variable:
    .. code-block:: bash

        WANDB_API_KEY=$YOUR_API_KEY

    :warning: **Heads up!** This step is *obligatory* if you want to see interactive plots and track experiments in the cloud. You'll still get local logs, but the full WandB dashboard experience requires this.
...
*   The project uses `Hydra <https://hydra.cc/>`_ for configuration management, allowing you to easily override parameters from the ``config/config.yaml`` file via command-line arguments.
...
For more detailed information on data generation scripts and their usage, please refer to the dedicated `README <scripts/README.md>`_.

Training Your Model
-------------------

To kick off a training run with the default settings (as defined in `config/config.yaml`_), simply execute:
...
One of the coolest features here is how easy it is to customize your experiments. You can modify any parameter from `config/config.yaml`_ by passing ``key=value`` pairs on the command line. For nested parameters, use dot notation (e.g., ``section.param=value``).
...
Configuration Options: Customize Your Experiments!
--------------------------------------------------
...
.. note::
    Only one of ``naive_oversample``, ``smote``, ``adasyn``, or ``naive_undersample`` can be active at a time.
